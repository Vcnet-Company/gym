...
//if request URL - reload page
			if(location.protocol!="file:"){
				if(location.search.replace("http://www.cbmcard.com/?", "http://www.cbmcard.com/")){
					var requestUrl = history.location || document.location;
					initLoad = true;
					loadContent(requestUrl.href.replace("http://www.cbmcard.com/?", "http://www.cbmcard.com/"), true);
				}else{
			...
			if(location.protocol!="file:"){
				...
				if(linkHref.indexOf("#")==-1){
					...
					if($(this).parents("form").length==0){
						...
						hrefDomen = hrefDomenSlice.slice(0, hrefDomenSlice.indexOf("http://www.cbmcard.com/"));
		...
		function popStateHcange(){
			if(location.protocol!="file:"){
				...
				if(!initLoad){
					...
					loadContent(requestUrl.href.replace("http://www.cbmcard.com/?", "http://www.cbmcard.com/"));
				}				
			}
		}
				...
				xhr = $.ajax({
					...
					beforeSend:function(){
						...
						if(url.indexOf("search.html")==-1){
							currButton=$("a[href='"+url.slice(url.lastIndexOf("http://www.cbmcard.com/")+1, url.length)+"']").parent();
						}else{
